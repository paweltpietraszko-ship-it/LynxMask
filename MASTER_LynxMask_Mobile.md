...
* **Warstwa obrazu (redakcja pikseli) — ZAMKNIĘTE 22.06.2026.** Decyzja właściciela (konsultacja Cursor, testy ręczne UI-2 + analiza use-case „zdjęcie dokumentu w sieci").
  ...
  * **Faza 0 (zamknięta 23.06.2026):** Share obrazu → ML Kit Face Detection → blur domyślnie → podgląd → share JPEG. Pliki: `ShareTargetActivity.kt`, `ImageRedactionPipeline.kt`.
  * **Faza 1 (zamknięta 23.06.2026):** ręczny prostokąt użytkownika na podpis/pieczątkę — `ImageRedactionScreen.kt` (drag → blur, tap → odkrycie, ostrzeżenie przed Share). ~~TODO-10~~ (placeholder onboardingu „zakryj palcem") — **anulowany**; zastąpiony przez F1 w flow Share obrazu.
